...
const int MX = 100001;
bool flag = true;
bool dfs(int src, vector<vector<int>> &adj, vector<int> &dir)
{
    for (auto neigh : adj[src])
    {
        if (dir[neigh] == dir[src])
        {
            return false;
        }
        if (dir[neigh] == 0)
...
void solve()
{
    ...
    while (minHeap.size())
    {
        ...
        ll currWeight = minHeap.top().first;
        minHeap.pop();
        ...
        for (auto neigh : adj[currNode])
        {
            ...
            if (distance[dest] > weight + currWeight)
            {
                distance[dest] = weight + currWeight;
                minHeap.push({distance[dest], dest});
